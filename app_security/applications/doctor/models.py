...
class CitaMedica(models.Model):
    ...
    class Meta:
        ...
        unique_together = ('fecha', 'hora_cita')  # Previene duplicidad por fecha y hora

    def save(self, *args, **kwargs):
        # Refuerzo: Evita que haya dos citas a la misma hora
        if CitaMedica.objects.exclude(pk=self.pk).filter(fecha=self.fecha, hora_cita=self.hora_cita).exists():
            raise ValueError("Ya existe una cita en la misma fecha y hora.")
        # Validación: solo permitir agendar en horas futuras del día actual o en días futuros
        from django.utils import timezone
        from datetime import datetime
        now = timezone.localtime()  # mejor usar localtime
        cita_datetime = datetime.combine(self.fecha, self.hora_cita)
        cita_datetime = timezone.make_aware(cita_datetime, now.tzinfo)
        if cita_datetime <= now:
            raise ValueError("No se puede agendar una cita en el pasado ni en una hora que ya pasó.")
        super().save(*args, **kwargs)

    @classmethod
    def get_admin_form(cls):
        return CitaMedicaForm
...
class CitaMedicaForm(forms.ModelForm):
    class Meta:
        ...
        fields = '__all__'
        widgets = {
            'fecha': forms.DateInput(attrs={'type': 'date', 'class': 'form-input', 'autocomplete': 'off'}),
        }

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # El campo real se oculta, el HTML de radios/checkboxes lo renderiza el JS
        self.fields['hora_cita'].widget = forms.HiddenInput(attrs={'data-hora-cita': 'true', 'id': 'id_hora_cita_hidden'})

    # Renderiza un div para los horarios dinámicos
    def render_horarios_div(self):
        return '<div id="id_hora_cita_radios"></div>'